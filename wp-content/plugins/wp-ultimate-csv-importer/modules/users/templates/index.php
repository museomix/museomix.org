...
$allcustomposts.=$value.',';
 ...
 ?>
   ...
   <input type='button' id='apply_element' name='apply_element' value='<?php echo __('Show','wp-ultimate-csv-importer'); ?>' class="btn btn-success" onclick='gotoelement(this.id);' style="margin-right:10px;margin-left:5px"/>
   </div>
   </td>
   </tr> 
   <?php
   ...
   foreach ($impCE->headers as $key => $value) {
	   ?>
		   ...
		   <span id='elementVal_<?php echo $key; ?>' > <?php echo $getrecords[0][$value]; ?> </span>
			...
			<td width = "180px;">
<input class="customfieldtext" type="text" id="textbox<?php print($count); ?>" name="textbox<?php print($count); ?>" TITLE="<?php echo __('Replace the default value','wp-ultimate-csv-importer'); ?>" style="display: none;" value="<?php echo $value ?>"/>
