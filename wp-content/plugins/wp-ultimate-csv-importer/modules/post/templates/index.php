...
<div style="width:100%;">
	<div id="accordion">
		<table class="table-importer">
											...
											<tr>
												<div style="float:right;">
													<input type='button' name='clearform' id='clearform'
														   title='<?php echo esc_attr__("Clear", 'wp-ultimate-csv-importer'); ?>'
														   value='<?php echo $impCE->reduceStringLength(esc_attr__("Clear", 'wp-ultimate-csv-importer'), 'Clear'); ?>'
			...
			<tr>
				<td>
					<form action='<?php echo esc_url(add_query_arg(array('page' => WP_CONST_ULTIMATE_CSV_IMP_SLUG.'/index.php', '__module' => sanitize_text_field($_REQUEST['__module']), 'step' => 'importoptions'), $impCE->baseUrl)); ?>'
						  name='mappingConfig' method="post" onsubmit="return import_csv();">
						...
						<div id='sec-two' <?php if (sanitize_text_field($_REQUEST['step']) !== 'mapping_settings') { ?> style='display:none;' <?php } ?> >
							<div class='mappingsection'>
								<h2>
									<div class="secondformheader"><?php echo esc_html__('Import Data Configuration', 'wp-ultimate-csv-importer'); ?></div>
								</h2>
								<?php
								...
								<?php if (isset($_REQUEST['__module']) && sanitize_text_field($_REQUEST['__module']) === 'custompost') { ?>
									<div class='importstatus'>
										...
										<div style='float:left;'>
											<label	id="importalign"><?php echo esc_html__('Select Post Type', 'wp-ultimate-csv-importer'); ?> </label>
								...
								<table style="font-size: 12px;" class="table table-striped" id='FIELDGRP'>
									<tr>
										<div align='center' style='float:right;'>
											...
											<input type='hidden' id='nonceKey' name='wpnonce' value='<?php echo sanitize_text_field($nonce_Key); ?>' />
											<input type='hidden' id='uploadedFile' name='uploadedFile' value="<?php if (isset($filename)) { echo sanitize_file_name($filename); } ?>" />
											<!-- real uploaded filename -->
											<input type='hidden' id='uploaded_csv_name' name='uploaded_csv_name' value="<?php if (isset($uploaded_csv_name)) { echo sanitize_file_name($uploaded_csv_name); } ?>" />
											<input type='hidden' id='stepstatus' name='stepstatus' value='<?php if (isset($_REQUEST['step'])) { echo sanitize_text_field($_REQUEST['step']); } ?>' />
											<input type='hidden' id='inline_image_location' name='inline_image_location' value='<?php if (isset($extracted_image_location)) { echo sanitize_text_field($extracted_image_location); } ?>' />
										</div>
									</tr>
									<?php
									$count = 0;
											...
											foreach ($impCE->defCols as $key => $value) {
												if (strstr($key, 'SEO: ')) {
													...
													<tr>
														<td class="left_align" style='width:53%; padding-left:170px;'>
															<input type='hidden'
																   name='seofieldname<?php print($count); ?>'
						...
						<div class="postbox" id="options" style=" margin-bottom:0px;">
							<div class="inside">
								<label id="importalign">
									<input type='radio' id='importNow' name='importMode' value='' onclick='choose_import_mode(this.id);' checked/> <?php echo esc_html__("Import right away", 'wp-ultimate-csv-importer'); ?>
								</label>
								<label id="importalign">
									<input type='radio' id='scheduleNow' name='importMode' value='' disabled/> <?php echo esc_html__("Schedule now", 'wp-ultimate-csv-importer'); ?>
								...
								<div id='importrightaway' style='display:block'>
									<form method="POST">
										<ul id="settings">
											<li>
												<label id="importalign">
													<input name='duplicatecontent' id='duplicatecontent' type="checkbox" value=""> <?php echo esc_html__('Detect duplicate post content', 'wp-ultimate-csv-importer'); ?>
												</label> <br>
												<input type='hidden' name='wpnoncekey' id='wpnoncekey' value='<?php echo sanitize_text_field($nonce_Key); ?>'/>
												<label id="importalign">
													<input name='duplicatetitle' id='duplicatetitle' type="checkbox" value=""> <?php echo esc_html__('Detect duplicate post title', 'wp-ultimate-csv-importer'); ?>
												</label> <br>
												<label id="importalign"><?php echo esc_html__('No. of posts/rows per server request', 'wp-ultimate-csv-importer'); ?></label>
												<span class="mandatory" style="margin-left:-13px;margin-right:10px">*</span>
												<input name="importlimit" id="importlimit" type="text" value="1" placeholder="10" onblur="check_allnumeric(this.value);" />
												</label>
										...
										<div id="ajaxloader" style="display:none">
											<img src="<?php echo esc_url(WP_CONST_ULTIMATE_CSV_IMP_DIR.'images/ajax-loader.gif');?>"> <?php echo esc_html__('Processing...', 'wp-ultimate-csv-importer'); ?>
										</div>
										<div class="clear"></div>
									</form>
								</div>
