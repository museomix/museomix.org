...
if ($options['addtoallowlist']=='Y') {
	kpg_sfs_check_admin(); // adds user to allowlist
}
// admin vrs. mu admin
...
add_filter('comment_row_actions','kpg_ss_row',1,2);
...
if ( function_exists('register_uninstall_hook') ) {
	// uncomment this our when we go to beta.
...
function sfs_handle_ajax() {
	wp_enqueue_script( 'stop-spammers', KPG_SS_PLUGIN_URL.'js/sfs_handle_ajax.js', false ); 
}
...
function kpg_ss_row($actions,$comment) {
	...
	$who="<a title=\"whois\" target=\"_stopspam\" href=\"http://lacnic.net/cgi-bin/lacnic/whois?lg=EN&query=$ip\"><img src=\"$whois\" width=\"16px\"/></a>";
	$stophand=KPG_SS_PLUGIN_URL.'images/stop.png';
	$stop="<a title=\"Check Stop Forum Spam (SFS)\" target=\"_stopspam\" href=\"http://www.stopforumspam.com/search.php?q=$ip\"><img src=\"$stophand\" width=\"16px\"/></a>";
