...
<form method="post" action="">
  ...
  <p class="submit">
    <input class="button-primary" value="Run Scan" type="submit" />
  </p>
</form>
<?php 
...
";
			...
			if (strpos($name,'_transient_feed_')===false) {
				...
				foreach ($badguys as $baddie=>$reas) {
					if (!(strpos($line,$baddie)===false)) {
						// bad boy
						$line=kpg_ss_make_red($baddie,$line);
						$reason.=$reas.' ';
					} 
				}
			}				
	...
	flush();
	if (kpg_ss_scan_for_eval()) $disp=true;;
...
function kpg_ss_scan_for_eval() {
	...
	for ($j=0;$j<count($phparray);$j++) {
		...
		if (strpos($phparray[$j],'threat_scan')===false&&strpos($phparray[$j],'threat-scan')===false) {
			$ansa=kpg_ss_look_in_file($phparray[$j]);
			if (count($ansa)>0) {
				$disp=true;
...
function kpg_ss_look_in_file($file) {
...
function kpg_ss_make_red($needle,$haystack) {
	// turns error red
